...
def _encode_messages(
    ...
    for message in messages:
        # If this is an assistant message from Anthropic with the same model_id
        # and it has raw_content, use it directly to preserve thinking blocks
        if (
            ...
            # raw_content is stored as dicts for serialization, cast to expected type
            result.append(
                {
                    "role": message.role,
                    "content": cast(
    ...
    with _base_utils.ensure_all_params_accessed(params) as param_accessor:
        ...
        if param_accessor.seed is not None:
            _base_utils.warn_unused_param(
                "seed", param_accessor.seed, "provider: anthropic"
            )
        if param_accessor.stop_sequences is not None:
            kwargs["stop_sequences"] = param_accessor.stop_sequences
        if param_accessor.thinking is not None:
            if param_accessor.thinking is False:
                kwargs["thinking"] = {"type": "disabled"}
            elif param_accessor.thinking is True:
